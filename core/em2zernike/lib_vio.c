...
void read_vol(char *vol_file, double *width, double *origx, double *origy, double *origz, unsigned *extx, unsigned *exty, unsigned *extz, double **phi) {
  ...
  if (n_range_viol2<n_range_viol0 && n_range_viol2<n_range_viol1) read_situs(vol_file, width, origx, origy, origz, extx, exty, extz, phi);
  else {
    read_mrc (vol_file, &orom, &cubic, &ordermode, &nc, &nr, &ns, &ncstart, &nrstart, &nsstart, &widthx, &widthy, &widthz, &xorigin, &yorigin, &zorigin, &alpha, &beta, &gamma, &phi_raw);
    permute_map (ordermode, nc, nr, ns, &nx, &ny, &nz, ncstart, nrstart, nsstart, &nxstart, &nystart, &nzstart, phi_raw, phi);
    permute_print_info(ordermode,nc,nr,ns,nx,ny,nz,ncstart,nrstart,nsstart,nxstart,nystart,nzstart);
    assert_cubic_map (orom, cubic, alpha, beta, gamma, widthx, widthy, widthz, nx, ny, nz, nxstart, nystart, nzstart, xorigin, yorigin, zorigin, width, origx, origy, origz, extx, exty, extz, phi);
    printf ("lib_vio> \n");
  }
...
void write_vol(char *vol_file, double width, double origx, double origy, double origz, unsigned extx, unsigned exty, unsigned extz, double *phi) {
  ...
  if (have_situs_suffix(vol_file)) write_situs(vol_file, width, origx, origy, origz, extx, exty, extz, phi);
  else write_mrc (1, vol_file, width, origx, origy, origz, extx, exty, extz, phi);
...
void read_situs(char *vol_file, double *width, double *origx, double *origy, double *origz, unsigned *extx, unsigned *exty, unsigned *extz, double **phi) {
	...
	int status = fscanf(fin, "%le %le %le %le %d %d %d", &dwidth, &dorigx, &dorigy, &dorigz, extx, exty, extz);

  if(status != 7)
  {
    printf("error reading situs file (bad header?!)\n");
    abort();
  }
...
void read_mrc (char *vol_file, int *orom, int *cubic, int *ordermode,
  ...
  *widthz = zlen / (double) mz;
  ...
  testa = (int)floor(100* *alpha+0.5); testb = (int)floor(100* *beta+0.5); testg = (int)floor(100* *gamma+0.5);
...
int test_situs (char *vol_file) {
  ...
  if (header_ok == 0) return 999999; /* worst case, can't read data */ 
    ...
    n_range_viols += (extx>5000); n_range_viols += (extx<0);
    n_range_viols += (exty>5000); n_range_viols += (exty<0);
    n_range_viols += (extz>5000); n_range_viols += (extz<0);
    n_range_viols += (width>2000); n_range_viols += (width<0);
    n_range_viols += (origx>1e6); n_range_viols += (origx<-1e6);
    n_range_viols += (origy>1e6); n_range_viols += (origy<-1e6);
    n_range_viols += (origz>1e6); n_range_viols += (origz<-1e6);
...
void permute_map (int ordermode, unsigned nc, unsigned nr, unsigned ns, unsigned *nx, unsigned *ny, unsigned *nz, 
...
void assert_cubic_map (int orom, int cubic, double alpha, double beta, double gamma, double widthx, double widthy, double widthz, unsigned nx, unsigned ny, unsigned nz, 
  ...
  if (test_registration(*porigx,*porigy,*porigz,*pwidth)==0) fprintf (stderr,"lib_vio> Input grid not in register with origin of coordinate system.\n");
}
...
void interpolate_skewed_map_to_cubic (double **pphiout, unsigned *pextx, unsigned *pexty, unsigned *pextz, 
  ...
  *pextz = (int) (ceil(uzmax/ *pwidth) - floor(uzmin/ *pwidth) + 1.5);
  ...
  for (indz=0;indz<(int)*pextz;indz++)
	...
	if (x0>=0 && x1<(int)nx && y0>=0 && y1<(int)ny && z0>=0 && z1<(int)nz) {	
	  a = gx-x0;
	  b = gy-y0;
	  c = gz-z0;
...
void write_mrc (int automode, char *vol_file, double pwidth, double porigx, double porigy,
  ...
  int i, wmaperr;
  long lskflg = 0;
  float skwmat11=1.0f,skwmat12=0.0f,skwmat13=0.0f,skwmat21=0.0f,skwmat22=1.0f,skwmat23=0.0f,skwmat31=0.0f,skwmat32=0.0f,skwmat33=1.0f,skwtrn1=0.0f,skwtrn2=0.0f,skwtrn3=0.0f;
  ...
  case 1: 
    break;
  case 2:
    if (clipped (pphi, pnvox, 127.0, -128.0)) printf ("lib_vio> Density values were clipped to fit signed 8-bit data format.\n");
    else printf ("lib_vio> Density values already fit signed 8-bit data format, no clipping necessary.\n");
    break;
  case 3:
    dscale = dmax/127.0;
    if (dmin/-128.0 > dscale) dscale = dmin/-128.0;
