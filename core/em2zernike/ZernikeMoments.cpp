...
//DD
		...
		std::cout << "PRINT gCoeffs_" << std::endl;
		for (int n=0; n<=order_; ++n)
    {
        ...
        for (int l=l0; l<=n; ++li, l+=2)
        {
            ...
            for (int m=0; m<=l; ++m)
            {
							for(int h=0; h<gCoeffs_[n][li][m].size();h++)
							{
								ComplexCoeffT cc = gCoeffs_[n][li][m][h];                                     
                ComplexT cvalue = cc.value_;

								std::cout << ((complex<double>)cvalue).real()
									<< "_" << ((complex<double>)cvalue).imag() << " ";						
							}
						}
						std::cout << std::endl;
				}
...
template<class VoxelT, class MomentT>
void ZernikeMoments<VoxelT,MomentT>::Compute ( ScaledGeometricalMoments<VoxelT,MomentT>& _gm)
{
	gm_ = _gm;
	Compute();
}
...
struct Values {
	...
	void print() {
		printf("%d %d %d %d %.2f %.2f\n", p_, q_, r_, valid, value_.real(), value_.imag());
	}
};
