...
mrf_one_body_features::mrf_one_body_features(
    ...
    double rmsd, double correlation, double overlap, string label) :
        alpha_z(alpha_z), beta_y(beta_y), gamma_x(gamma_x),
...
mrf_two_body_features::mrf_two_body_features(double alpha_z_left,
    ...
    soroban_score physics_score, string label_left, string label_right) :
        ...
        physics_score(physics_score), label_left(label_left),
...
void mrf_two_body_features::write(ofstream& data_stream) {
  ...
  data_stream << "\t" << alpha_z_left << "\t" << beta_y_left << "\t"
              << gamma_x_left << "\t" << translate_x_left << "\t"
              << translate_y_left << "\t" << translate_z_left << "\t"
              << alpha_z_right << "\t" << beta_y_right << "\t" << gamma_x_right
              << "\t" << translate_x_right << "\t" << translate_y_right << "\t"
              << translate_z_right << "\t" << label_left << "\t"
              << label_right << endl;
}

void mrf_two_body_features::write_header(ofstream& data_stream) {
  data_stream << "RMSD\tPhysicsScore\tvdw\tvdw_a\tvdw_r\t"
              << "elec\telec_sr_a\telec_lr_a\telec_sr_r\telec_lr_r\t"
              << "hb\tsolv\tsasa\tacp\t"
              << "Alpha_z_l\tBeta_y_l\tGamma_x_l\ttx_l\tty_l\ttz_l\t"
              << "Alpha_z_r\tBeta_y_r\tGamma_x_r\ttx_r\tty_r\ttz_r\t"
              << "Unit_l\tUnit_r" << endl;
}
