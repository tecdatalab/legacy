...
class pdb
{
  private:
    ...
    // This is to avoid re-computing it in programs where the centroid is
    // required several times. Every time precompute_centroid is called it will
    // reset these values.
    double centroid_x, centroid_y, centroid_z;
    // Flag read by get_centroid to determine if precompute_centroid has been
    // called before. If it hasn't centroid_? have not been assigned so far
    // Upon construction it's set to false.
  ...
  /*
   * Allocates an empty PDB and merges two PDB's respecting
   * the residue number ordering in each of the PDB's.
   * Originally this process was done by merely concatenating
   * the 2 atom vectors. However, the alignment functions assume
   * that residue numbers are consecutive in each pdb instance
  ...
  static pdb* merge(pdb& first_pdb, pdb& second_pdb);
    ...
    /*
     * Returns the lowest and highest x, y and z values (in PDB coordinate
     * space). In practice, this is a bounding box enclosing the PDB.
     */
    void get_boundaries(float* min_x, float* min_y, float* min_z,
                        float* max_x, float* max_y, float* max_z);

		/*
		 * This method will remove all the atoms that are farther away than "distance"
		 * from the "partner" structure. In other words, only atoms that are closer than
		 * "distance" to at least one atom in the partner will be kept
		 */
		void remove_distant_atoms(vector<atom>* partner, double distance);

    /*
     * Compare this.atoms against matched_pdb.atoms and find the c-alpha atoms
     * that exactly match between them. A match is defined as two c-alphas that 
     * have the same residue identifier, same 3-letter code and (optionally)
     * the same chain ID.
     *
     * When calling somepdb.get_calphas(other_pdb,...) it is expected that all
     * atoms in somepdb have a correspondence in other_pdb. e.g. if there is
     * a smaller PDB out of the two then the call should be
     * small.get_matching_calphas(bigger,...).
     *
     * Returns as output values copies of the atoms from this instance and also
     * copies from the matched pdb.
     *
     * Note: ignore_chain_id==true => only residue ID and 3-letter code need
     * to be the same for a match to happen.
     */
    void get_matching_calphas(pdb* matched_pdb, bool ignore_chain_id, vector<atom>* atoms_this, vector<atom>* atoms_matched);

  /*
   * Go through all non-hydrogen atoms and calculate the geometric center
   * (centroid) by averaging the coordinates from each axis, returning the
  ...
  // Return the precomputed centroid_? values. If they have not been
  // assigned before, it calls precompute_centroid to do so
