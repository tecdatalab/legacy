...
class atom
{
	...
	atom(string m_type, string m_residue, string m_chain, int m_rid, double* m_xyz, double m_charge, double m_rad, int m_acp_type, charmm_param& m_chpar,
	...
	string tostring() {
		char buf[200];
		//sprintf(buf, "%d %s %d %f %f %f %s %s %f %f %f %d %f %f", anum, atype.c_str(), rnum , axyz[0], axyz[1], axyz[2],
		//							residue.c_str(), chain.c_str(), atom_solv,
		//						arad, atomcharge, acp_type, occupancy, temp_factor);
		...
		//return string(buf) + " " + chpar.tostring();
	}
