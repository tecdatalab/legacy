...
void compute_vdw_energy(vector<atom>& R, vector<atom>& L, ANNkd_tree* T, double& vdw,
    double& vdw_attr, double& vdw_rep)
{
...
// formula taken from pyDOCK Coulombic electrostatics with distance-dependent
// dielectric constant (eps = 4r), explicitly calculated for all intermolecular atom pairs,
// with q atomic charges from CHARMM19
// electrostatics
// weighted sum of short and long range attractive and repulsive forces
// charges of only the most polar groups are considered (ARG, LYS,
// ASP, GLU, C-terminus, and N-terminus).
// a charge of -0.5 on each of the d-oxygen atoms in ASP and the e-oxygen atoms in Glu,
// a charge of +0.5 on the eta-nitrogen atoms in Arg, and a
// full positive charge +1.0 on the z-nitrogen atom in Lys.

// use CHARMM19 charges for distances < 5.0 (short range)
// long range use only fully charged side chains
...
void compute_LZ_solvation(vector<atom>& R, vector<atom>& L, double& solv)
{
	...
	//printf("%s %s %d %d\n", R[i].atype.c_str(), R[i].residue.c_str(), R[i].INTF, R[i].chpar.valset);
    	...
    	// This line was added to correct a calculation error, since the DGREF
