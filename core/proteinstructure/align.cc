                               ...
                               point_transformation* optimal_transformation=NULL)
{
  ...
  // The copies are fed into a view
	gsl_matrix_const_view input_view =
      gsl_matrix_const_view_array(input_coords, input->size(), 3);
	gsl_matrix_const_view target_view =
      gsl_matrix_const_view_array(target_coords, target->size(), 3);
  ...
  if(rmse != -1) {
    ...
    for(size_t transformed_index = 0;
        transformed_index < input_to_transform->size(); transformed_index++) {
      atom transformed_atom = (*input_to_transform)[transformed_index];
      if(debug) {
        cerr << "Transforming " << transformed_atom.residue
             << " " << transformed_atom.rnum << endl;
      }
  ...
  if (optimal_transformation) {
    *optimal_transformation =
        gsl_transformation_to_point_transformation(R, t);
  }
