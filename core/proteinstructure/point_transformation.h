...
// Provides a way of storing a rotation/translation
// and then applying it to a point, by calling the
// transform method, which returns the x,y,z coordinates
...
// The class performs the appropriate adjustments internally
// when libraries require radians
class point_transformation
{
 private:
	...
	// represents a sequence of roll, pitch and yaw
	// rotations, described by the angles provided
	// It's constructuted by multiplying
	// 	Rz(alpha) * Ry(beta) * Rx(gamma)
	// Where each of the R? matrices are the standard
	// rotation matrices along the corresponding axis
	// Following transformation matrix standards, a new vector
	// v is transformed by multiplying Rv (i.e the matrix multiplies
	// on the left)
	double rotation_matrix[3][3];
	// Returning the points to their original position is a typical
	// operation in our code. Given that matrix multiplication
	// is not commutative, we precalculate the inverse transformation
	// (i.e. Rx(-gamma) * Ry(-beta) * Rz(-alpha))
	double inverse_rotation_matrix[3][3];
	// After applying rotations, points are translated by these
	// x,y,z magnitudes
 ...
 public:
  ...
  void set_rotation(double matrix[3][3]);
	...
	void set_translation(double translate_x, double translate_y,
						double translate_z);
	// Applies rotation_matrix*[in_x, in_y, in_z] + translation
	// In other words, it first rotates and then translated
	void transform(double in_x, double in_y, double in_z,
					double* out_x, double* out_y, double* out_z);
	// Inverse process:
	...
	void invert_transform(double in_x, double in_y, double in_z,
					double* out_x, double* out_y, double* out_z);
...
class point_transformation_sequence
{
 ...
 public:
  ...
  void add(double alpha_z, double beta_y, double gamma_x,
           double translate_x, double translate_y, double translate_z);
  void add_translation(double translate_x, double translate_y,
                       double translate_z);
...
class point_transformation_generator
{
 public:
  ...
  bool is_finished();
  // Take the current transformation_parameters, create a point transformation
  // and return it. It will also modify transformation_parameters to have them
  // ready for the next call.
  // Thus this method always computes the transformation that will come next
  // before returning.
  point_transformation next();
  // Returns the total number of transformations that will be returned by next
  // before we finish sampling the space.
  // TODO
//  unsigned long get_total_transformations();
 private:
  // Index from 0 to 5 (0-2 alpha,beta,gamma and 3-5 x,y,z)
  // that indicates which of the six degrees of freedom needs to be updated next
