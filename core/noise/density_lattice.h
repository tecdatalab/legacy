...
class density_lattice : public point_lattice
{
 ...
 public:
  ...
  double calculate_correlation(density_lattice& compared_lattice,
	...
	//Additional code to remove noise from input EM file

	//Removes density values corresponding to "noise" using the following functions
	//minimum is "dmin" - the minimum value of the file
	void remove_noise(float minimum);

	//Checks the 6 neighbors of coordinate (x,y,z) to get their group numbers
	std::set<unsigned int> check_neighbors(unsigned long x,
			unsigned long y, unsigned long z,
			unsigned int ***groups);


	//Checks the 6 neighbors of (x,y,z) to determine which group the new value[x][y][z] gets assigned to
	void assign_group(unsigned long x,
	...
	//Overwrites the smaller "noise" groups to have the dmin value
	void delete_small_groups(unsigned int ***groups, float dmin,
			std::map<unsigned int, unsigned int>& count, merge_manager& merge_groups);
};
