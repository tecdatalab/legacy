...
ZernikeDescriptor<T, TIn>::ZernikeDescriptor()
{
	...
	T* result = new T[_dim_*_dim_*_dim_];

	int i = 0;

	ZernikeDescriptor<double, float>::ComplexT3D temp; 
	temp.resize(_dim_);
	//sphere of r=20 dim = 50 
	for (int a=0; a<_dim_; ++a)
	{
		temp[a].resize(_dim_);
		for (int b=0; b<_dim_; ++b)
		{
			temp[a][b].resize(_dim_);
			for (int c=0; c<_dim_; ++c)
			{
				++i;
				if ((a- center )*(a- center )+(b- center )*(b- center )
						+(c- center )*(c - center)< vol){
					result[(c*_dim_ + b)*_dim_ + a] = 1;
				} else {
					result[(c*_dim_ + b)*_dim_ + a] =  0;
				}
			}
		}
	}
	...
	SaveInvariants ("1_2.txt");
	
	ZernikeDescriptor<double, float>::ComplexT3D recon_result;
	
	recon_result.resize(dim_);
	for(int i = 0; i < dim_; i++){
		recon_result[i].resize(dim_);
		for(int j = 0; j < dim_; j++){
			recon_result[i][j].resize(dim_);
		}
	}
	
	Reconstruct(recon_result, 1, order_, 1, order_ );
	
//	zm_.PrintGrid( "2_2.txt" , recon_result);
	...
	for (int a=0; a<_dim_; ++a)
	{
		for (int b=0; b<_dim_; ++b)
		{
			...
			{
				++i;
				if ((a-5)*(a-5)+(b-5)*(b-5)+(c-5)*(c-5)<40 && (a-5)*(a-5)+(b-5)*(b-5)+(c-5)*(c-5)> 10){
					result[(c*_dim_ + b)*_dim_ + a] = 1;
				} else {
					result[(c*_dim_ + b)*_dim_ + a] =  0;
				}
...
T* ZernikeDescriptor<T, TIn>::ReadGrid (const char* _fname, int& _dim_)
{
    ...
    while (infile.read ((char*)(&temp), sizeof (TIn)))
    {
		 ...
		 if(temp == '1')
        	tempGrid.push_back (1000);
		 if(temp == '2')
        	tempGrid.push_back (666);
		 if(temp == '3')
        	tempGrid.push_back (333);
    }  

    int d = tempGrid.size ();

	// BioLab
	cout << "\nRead Grid size : " << d << endl;
...
template<class T, class TIn>
void ZernikeDescriptor<T, TIn>::Reconstruct (char * file, ComplexT3D& _grid, int _minN, int _maxN, int _minL, int _maxL)
{
    // the scaling between the reconstruction and original grid
    T fac = (T)(_grid.size ()) / (T)dim_;

    zm_.Reconstruct (file,
					 _grid,         // result grid
...
void ZernikeDescriptor<T,TIn>::SaveInvariants (const char* _fName)
{
	std::ofstream outfile (_fName, std::ios_base::binary |
		std::ios_base::out);
	float temp;
