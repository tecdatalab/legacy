...
bool ptalign(gsl_matrix_const_view x, gsl_matrix_const_view y, gsl_matrix *R, gsl_vector *t, double *rmse)
{
...
// align target and query using vertices
bool ptalign(vector<vector<double> >& A, vector<vector<double> >& B, double* tvect)
{
    int npts = (int)A.size();
    const size_t n1 = npts;
    const size_t n2 = 3;
    ...
    double rmse;
    bool retVal = false;
    if((retVal = ptalign(xv, yv, R, t, &rmse)))
    {  
        T3Matrix mR;
  
        for(int l=0;l<3;l++)
            for(int j=0;j<3;j++)
                mR[l][j] = gsl_matrix_get(R,l,j);
  
        double mT[3];
        for(int l=0;l<3;l++)
            mT[l] = gsl_vector_get(t, l);
