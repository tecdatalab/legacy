...
foreach my $directory (@ls_results) {
  ...
  if ($#xml_dir >= 0 && $#image_dir >= 0 && $#map_dir >= 0) {
    ...
    for my $current_image_index (0 .. $#image_dir) {
	  ...
	  my @image_name_array = split(/\./, $base_image_filename);
	  my $base_image_id = $image_name_array[0];

      # There are a few names that people use more frequently.
      # If none of them are present then we'll just take the first image.
      if ($base_image_id eq $emdbid ||
          $base_image_id eq ("EMD-" . $emdbid) ||
          $base_image_id eq ("emd_" . $emdbid) ||
          $base_image_id eq ("emd-" . $emdbid)) { 
	...
	my $xml_file_index = -1;
    for my $current_xml_index (0 .. $#xml_dir) {
      my $base_xml_filename =
          get_filename_from_dir_entry($xml_dir[$current_xml_index]);

      my @xml_name_array = split(/\./, $base_xml_filename);
	  my $base_xml_id = $xml_name_array[0];

	  # Check index of xml file in format emd-$emdbid.xml. 
	  # If not present, just take the first xml file.
	  if ($base_xml_id eq ("emd-" . $emdbid)) { 
        $xml_file_index = $current_xml_index;
        last;
      }
    }
    if ($xml_file_index == -1) {
    ...
    print $output_file "$emdbid" .
          ...
          "\t" . $xml_cwd . get_filename_from_dir_entry($xml_dir[$xml_file_index]) .
          "\t" . $image_cwd .
