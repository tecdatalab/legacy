...
# Script: add_length_complex.pl
# Description: Usage: add_length_complex.pl <complex directory> <basic pdb_latest file> <output filename>
# Author: YI XIONG
...
while ( <IN_BASIC> ) {
	...
	$line = $_;
	chomp($line);
	if ( $line =~ /^....\.pdb\.inv/  || $line =~ /^........\.pdb\.inv/) {
		
		if ( $line =~ /^....\.pdb\.inv/ ) {
			$pdb_id = substr( $line, 0, 4 );	
		} else {
			$pdb_id = substr( $line, 0, 8 );
		}
