...
# Script: perl idlistdistance.pl falseidlist1 singlechaincomplexidlist1
# Description: atom distance
# Author: Yi Xiong
# Email: xiongyi128@gmail.com
# Date: Apr.8.2013

chdir("/bio/kihara-web/www/db/complexdistance");
...
$singlechaincomplexidlistfile=$ARGV[1];

open(OUTSINGLE, ">$singlechaincomplexidlistfile");
foreach my $m ( 0 .. $#idarray ) {
	...
	if ( $lenhash < 2 ) {
		...
		print OUTSINGLE $idarray[$m] . "\n";
	} else {
		
		 open( OUT, ">distance/$idarray[$m]" );
	  endloop: foreach my $n ( 0 .. $#chainidarray ) {
			...
			foreach my $p ( $pinit .. $#chainidarray ) {
				...
				print OUT $firstchainid. "\t" . $firstcontactnum. "\t" .$secondchainid ."\t". $secondcontactnum . "\n";
			}
		}
	close(OUT);
	}
